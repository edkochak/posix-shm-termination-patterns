...
ifeq ($(UNAME_S),Linux)
    ...
    # macOS не требует библиотеку rt
    LDFLAGS = -pthread
endif
...
# Очистка LaTeX временных файлов
clean-tex:
	rm -f *.aux *.log *.toc *.out *.synctex.gz

# Полная очистка (включая программы и LaTeX файлы)
clean-all: clean clean-tex
	rm -f report.pdf

# LaTeX компиляция - запуск дважды для правильного формирования содержания
report: report.tex
	pdflatex -interaction=nonstopmode report.tex || true
	pdflatex -interaction=nonstopmode report.tex
	rm -f *.aux *.log *.toc *.out *.synctex.gz

# Запуск программ с задержкой для правильной инициализации разделяемой памяти
run_signal:
...
# Для повторных запусков - очистка ресурсов перед запуском
clean_resources:
